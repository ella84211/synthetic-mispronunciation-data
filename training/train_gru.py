"""
Mispronunciation Detection — BiGRU Training & Inference Script
==============================================================

Architecture: stacked BiGRU encoder with three prediction heads.

  INPUT REPRESENTATION
  --------------------
  The pronounced sequence is expanded with <GAP> tokens interleaved between
  every adjacent phone pair and at word edges. This gives the BiGRU explicit
  ...
  ARCHITECTURE NOTES
  ------------------
  - Stacked BiGRU (default: 2 layers, 256 hidden units per direction)
  - Hidden size per position = 2 * hidden_size (concatenation of fwd + bwd)
  - Dropout applied between GRU layers and before each classification head
  - Class weights applied to phone and gap losses to handle imbalance

USAGE
-----
  # Train
  python train_gru.py train \\
      --train  data/train.json   \\
      --output runs/bigru_real_only

  # Inference only
  python train_gru.py predict \\
      --test   data/test.json \\
      --output runs/bigru_real_only
  ...
  python train_gru.py train \\
      --val    data/val.json   \\
      --output bigru_experiments/bigru_real_only \\
...
class BiGRUMispronunciationModel(nn.Module):
    """
    Stacked BiGRU encoder with three classification heads.

    Hidden state at each position is the concatenation of the forward and
    backward GRU outputs, giving a vector of size 2 * hidden_size.
    All three heads operate on this combined representation.

    Compared to the BiLSTM variant, the GRU uses a single hidden state
    (no cell state), reducing parameter count while retaining strong
    ...
    def __init__(
        ...
        # GRU replaces LSTM: no cell state, otherwise identical interface
        self.gru = nn.GRU(
    ...
    def encode(
        ...
        # Pack padded sequence so the GRU ignores padding
        lengths = attention_mask.sum(dim=1).cpu()
        packed  = nn.utils.rnn.pack_padded_sequence(
            x, lengths, batch_first=True, enforce_sorted=False
        )
        # GRU returns (packed_output, hidden); no cell state to unpack
...
def train_epoch(
    model:     BiGRUMispronunciationModel,
    loader:    DataLoader,
...
def main():
    parser = argparse.ArgumentParser(
        description="Train and evaluate a BiGRU mispronunciation detection model."
    )
    ...
    tr.add_argument("--embedding_dim", type=int,   default=128,
                    help="Embedding dimension fed into the GRU.")
    tr.add_argument("--hidden_size",   type=int,   default=256,
                    help="GRU hidden size per direction (output is 2x this).")
